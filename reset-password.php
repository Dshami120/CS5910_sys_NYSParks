...
if (!$resetUserId) { flash_set('error', 'Verify your account first.'); redirect('forgot-password.php'); }
...
if (!$resetUser) { unset($_SESSION['reset_user_id']); flash_set('error', 'We could not find an active account to reset. Please verify your account again.'); redirect('forgot-password.php'); }
// Handle submitted form actions.
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $password = post('password');
    $confirm = post('confirm_password');
    if (strlen($password) < 8) { flash_set('error', 'Password must be at least 8 characters.'); redirect('reset-password.php'); }
    if ($password !== $confirm) { flash_set('error', 'Passwords do not match.'); redirect('reset-password.php'); }
    $db->prepare("UPDATE users SET password_hash = ? WHERE id = ?")->execute([password_hash($password, PASSWORD_DEFAULT), $resetUserId]);
    ...
    <section class="container">
      <section class="auth-panel p-4 p-lg-5">
        ...
        <form method="post">
          ...
          <input type="password" name="password" class="form-control form-control-lg mb-3" placeholder="New password" required /><label class="form-label">Confirm password</label><input type="password" name="confirm_password" class="form-control form-control-lg mb-4" placeholder="Confirm new password" required />
