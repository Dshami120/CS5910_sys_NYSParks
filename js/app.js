  ...
  if (aiForm && aiInput && aiMessages) {
    ...
    const setLoading = (loading) => {
      if (submitButton) {
        ...
        submitButton.innerHTML = loading ? '<span class="spinner-border spinner-border-sm me-1" aria-hidden="true"></span> Sending' : '<i class="bi bi-send-fill me-1"></i> Send Message';
      }
    ...
    const requestAiReply = async (prompt) => {
      setLoading(true);
      if (aiStatus) aiStatus.textContent = 'Sending your question to the NYS Parks Guide...';
      try {
        ...
        if (aiStatus) aiStatus.textContent = 'Response generated by the NYS Parks Guide.';
      } catch (error) {
        addMessage('assistant', error.message || 'The AI Guide could not answer right now. Please try again later.');
        if (aiStatus) aiStatus.textContent = 'The AI Guide is temporarily unavailable. Please try again later.';
      } finally {
        setLoading(false);
        aiInput.focus();
      }
    ...
    clearAiChat?.addEventListener('click', () => {
      ...
      if (aiStatus) aiStatus.textContent = 'Choose a sample prompt or type your own question below.';
    });
  }
});
