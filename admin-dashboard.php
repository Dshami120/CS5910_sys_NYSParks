...
// Build dashboard chart rows.
$adminChartRows = [];

foreach ($db->query("SELECT DATE(b.start_datetime) AS date, p.name AS park, 'bookings' AS metric, 1 AS value FROM bookings b JOIN parks p ON p.id=b.park_id WHERE b.booking_status IN ('approved','confirmed') ORDER BY b.start_datetime")->fetchAll() as $row) {
    $adminChartRows[] = $row;
}

foreach ($db->query("SELECT DATE(e.start_datetime) AS date, p.name AS park, 'events' AS metric, 1 AS value FROM events e JOIN parks p ON p.id=e.park_id WHERE e.event_status='published' ORDER BY e.start_datetime")->fetchAll() as $row) {
    $adminChartRows[] = $row;
}

foreach ($db->query("SELECT DATE(a.registered_at) AS date, p.name AS park, 'attendance' AS metric, a.guest_count AS value FROM attendance a JOIN events e ON e.id=a.event_id JOIN parks p ON p.id=e.park_id WHERE a.attendance_status IN ('registered','attended') ORDER BY a.registered_at")->fetchAll() as $row) {
    $adminChartRows[] = $row;
}

foreach ($db->query("SELECT DATE(py.created_at) AS date, COALESCE(p.name, 'No park') AS park, 'donations' AS metric, py.amount AS value FROM payments py LEFT JOIN bookings b ON b.id=py.booking_id LEFT JOIN parks p ON p.id=b.park_id WHERE py.payment_type='donation' AND py.payment_status='completed' ORDER BY py.created_at")->fetchAll() as $row) {
    $adminChartRows[] = $row;
}

// Set traffic availability flag.
$trafficAvailable = false;

// Prepare news lookup data.
...
if (!$selectedNews && $newsItems) {
    $selectedNews = $newsItems[0];
}

// Load employee and park options.
...
<?php include __DIR__ . '/includes/header.php'; ?>
    ...
    <main class="container py-5">
        ...
        <section class="row g-4 mb-4" id="attendance-summary">
            ...
            <!-- Registered RSVP count -->
            <div class="col-md-6 col-xl-3">
                <article class="admin-stat-card">
                <span class="admin-stat-icon icon-green">
            ...
            <!-- Upcoming RSVP guest count -->
            <div class="col-md-6 col-xl-3">
                <article class="admin-stat-card">
                <span class="admin-stat-icon icon-blue">
            ...
            <div class="col-md-6 col-xl-3">
                <article class="admin-stat-card">
                <span class="admin-stat-icon icon-purple">
            ...
            <!-- Total RSVP guest count -->
            <div class="col-md-6 col-xl-3">
                <article class="admin-stat-card">
                <span class="admin-stat-icon icon-orange">
        ...
        <section class="row g-4 mb-4">
            ...
            <div class="col-lg-7">
                <article class="admin-panel h-100">
                            ...
                            <div>
                                ...
                                <select class="form-select admin-filter-select" id="adminChartPark">
                                    ...
                                    <?php foreach ($parks as $park): ?>
                                        <option value="<?= e($park['name']) ?>"><?= e($park['name']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                    ...
                    <?php if (!$trafficAvailable): ?>
                        <p class="text-muted small mt-3 mb-0">Traffic is not available yet because there is no site_visits or page_views table.</p>
                    <?php endif; ?>
                </article>
            </div>
            ...
            <div class="card-body p-4 p-lg-5">
                ...
                <section class="list-shell mb-0">
                    ...
                    <?php foreach ($recentAttendance as $record): ?>
                        <section class="list-row p-4">
                            <section class="row g-3 align-items-start">
                                ...
                                <span class="status-pill <?= booking_status_class($record['attendance_status']) ?>">
                                    <?= e(ucfirst(str_replace('_',' ', $record['attendance_status']))) ?>
                                </span>
                                </article>
                            </section>
                        </section>
                    ...
                    <?php if (!$recentAttendance): ?>
                        <section class="p-4 text-muted">No attendance records yet.</section>
                    <?php endif; ?>
                </section>
            </div>
        </section>
