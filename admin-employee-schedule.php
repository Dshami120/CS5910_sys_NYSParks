...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($action, ['create_schedule','update_schedule'], true)) {
        ...
        if (!isset($scheduleStatusLabels[$scheduleStatus])) { $scheduleStatus = 'scheduled'; }
...
$schedulesByStatus = [];
foreach ($scheduleStatusLabels as $key => $label) { $schedulesByStatus[$key] = []; }
foreach ($allSchedules as $row) { $schedulesByStatus[$row['schedule_status']][] = $row; }
        ...
        <article class="col-xl-4">
          <section class="form-panel p-4 h-100">
            <div class="d-flex justify-content-between align-items-start gap-3 mb-3">
              <h2 class="h5 fw-bold mb-0"><?= $scheduleReadOnly ? 'View Schedule' : ($selectedSchedule ? 'Update Current Schedule' : 'Create New Schedule') ?></h2>
            ...
            <form method="get" class="mb-3">
              ...
              <?php if ($selectedSchedule): ?><input type="hidden" name="schedule_id" value="<?= (int)$selectedSchedule['id'] ?>" /><?php endif; ?>
              <select class="form-select" name="schedule_filter" onchange="this.form.submit()">
                ...
                <?php foreach ($scheduleStatusLabels as $key => $label): ?>
                  <option value="<?= e($key) ?>" <?= $scheduleFilter === $key ? 'selected' : '' ?>><?= e($label) ?> shifts</option>
                <?php endforeach; ?>
              </select>
            </form>
            <form method="get" class="mb-3">
                  ...
                  <?php if (!empty($schedulesByStatus[$statusKey])): ?>
                    <optgroup label="<?= e($statusLabel) ?> Shifts">
                      <?php foreach ($schedulesByStatus[$statusKey] as $row): ?>
                        <option value="<?= (int)$row['id'] ?>" <?= $selectedSchedule && (int)$selectedSchedule['id'] === (int)$row['id'] ? 'selected' : '' ?>><?= e($row['employee_name']) ?> — <?= e(format_date($row['shift_date'])) ?> <?= e(date('g:i A', strtotime($row['start_time']))) ?></option>
                      <?php endforeach; ?>
                    </optgroup>
                  <?php endif; ?>
                <?php endforeach; ?>
            ...
            <?php if ($scheduleReadOnly): ?><div class="alert alert-warning py-2">Cancelled schedules and disabled employee schedules are view-only. Create a new schedule if needed.</div><?php endif; ?>
            <form method="post">
              ...
              <select name="employee_id" class="form-select mb-3" required <?= $scheduleReadOnly ? 'disabled' : '' ?>>
                ...
                <?php foreach ($employees as $emp): ?><option value="<?= (int)$emp['id'] ?>" <?= (int)($selectedSchedule['employee_id'] ?? 0)===(int)$emp['id'] ? 'selected' : '' ?>><?= e(full_name($emp)) ?></option><?php endforeach; ?>
              </select>
              <label class="form-label">Park</label>
              <select name="park_id" class="form-select mb-3" required <?= $scheduleReadOnly ? 'disabled' : '' ?>>
                <option value="">Select park</option>
                <?php foreach ($parks as $park): ?><option value="<?= (int)$park['id'] ?>" <?= (int)($selectedSchedule['park_id'] ?? 0)===(int)$park['id'] ? 'selected' : '' ?>><?= e($park['name']) ?></option><?php endforeach; ?>
              </select>
              ...
              <section class="row g-3">
                <article class="col-6"><label class="form-label">Start</label><input type="time" name="start_time" class="form-control" value="<?= e(substr((string)($selectedSchedule['start_time'] ?? ''), 0, 5)) ?>" required <?= $scheduleReadOnly ? 'disabled' : '' ?> /></article>
                <article class="col-6"><label class="form-label">End</label><input type="time" name="end_time" class="form-control" value="<?= e(substr((string)($selectedSchedule['end_time'] ?? ''), 0, 5)) ?>" required <?= $scheduleReadOnly ? 'disabled' : '' ?> /></article>
              </section>
              ...
              <select name="schedule_status" class="form-select mb-3" <?= $scheduleReadOnly ? 'disabled' : '' ?>>
                <?php foreach ($scheduleStatusLabels as $key => $label): ?>
                  <option value="<?= e($key) ?>" <?= ($selectedSchedule['schedule_status'] ?? 'scheduled') === $key ? 'selected' : '' ?>><?= e($label) ?></option>
                <?php endforeach; ?>
              </select>
              ...
              <button type="submit" class="btn btn-success rounded-pill w-100 fw-semibold" <?= $scheduleReadOnly ? 'disabled' : '' ?>><?= $selectedSchedule ? 'Update Current Schedule' : 'Create New Schedule' ?></button>
            </form>
          </section>
        </article>
        ...
        <article class="col-xl-8">
          <section class="list-shell mb-4">
            ...
            <section class="<?= $i < count($schedules) - 1 ? 'list-row' : '' ?> p-4">
              <section class="row g-3 align-items-center">
                <article class="col-md-2"><?= e($row['employee_name']) ?></article>
                <article class="col-md-2"><?= e($row['park_name']) ?></article>
                <article class="col-md-2 text-muted"><?= e(format_date($row['shift_date'])) ?></article>
                <article class="col-md-2 text-muted"><?= e(date('g:i A', strtotime($row['start_time']))) ?>–<?= e(date('g:i A', strtotime($row['end_time']))) ?></article>
                <article class="col-md-2 text-muted"><?= e($row['assignment']) ?></article>
                <article class="col-md-1 text-muted"><?= e($scheduleStatusLabels[$row['schedule_status']] ?? ucfirst($row['schedule_status'])) ?></article>
                <article class="col-md-1 d-flex flex-wrap gap-5">
                  <a class="btn btn-sm btn-outline-dark" href="<?= e(schedule_admin_path(['schedule_id' => (int)$row['id']])) ?>"><?= (($row['schedule_status'] ?? '') === 'cancelled' || ($row['account_status'] ?? '') === 'disabled') ? 'View' : 'Edit' ?></a>
            ...
            <?php endforeach; ?>
            <?php if (!$schedules): ?><section class="p-4 text-muted">No schedules have been created yet.</section><?php endif; ?>
          </section>
