...
$approvedToday = (int) $db->query("SELECT COUNT(*) FROM bookings WHERE booking_status='approved' AND DATE(reviewed_at)=CURDATE()")->fetchColumn();
$feesQueued = (float) $db->query("SELECT COALESCE(SUM(reservation_fee),0) FROM bookings WHERE booking_status='pending'")->fetchColumn();
$rows = $db->query("SELECT b.*, p.name AS park_name, CONCAT(u.first_name,' ',u.last_name) AS requester FROM bookings b JOIN parks p ON p.id=b.park_id JOIN users u ON u.id=b.client_id ORDER BY b.created_at DESC")->fetchAll();
?><!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
