          ...
          <section class="list-shell">
              <section class="p-4 border-bottom">
                  <section class="row g-3">
                      <article class="col-lg-2 fw-bold">Requester</article>
                      ...
                      <article class="col-lg-2 fw-bold">Actions</article>
                  </section>
              </section>
              ...
              <section class="<?= $i < count($rows) - 1 ? 'list-row' : '' ?> p-4">
                  <section class="row g-3 align-items-center">
                      <article class="col-lg-2"><?= e($row['requester']) ?></article>

                      <article class="col-lg-2"><?= e($row['title']) ?></article>
                      <article class="col-lg-2 text-muted"><?= e($row['park_name']) ?></article>
                      <article class="col-lg-2 text-muted"><?= format_date(substr((string)$row['start_datetime'],0,10)) ?></article>
                      <article class="col-lg-1 text-muted">$<?= number_format((float) $row['reservation_fee'], 0) ?></article>
                      <article class="col-lg-2 text-muted"><?php if ($row['payment_status']): ?><span class="status-pill <?= booking_status_class($row['payment_status']) ?>"><?= e(ucfirst($row['payment_status'])) ?></span><br><small><?= e(str_replace('_',' ', (string)$row['payment_method'])) ?></small><?php else: ?>No payment<?php endif; ?></article><article class="col-lg-2"><section class="d-flex gap-2"><form method="post" class="d-flex flex-wrap gap-2 mb-0"><input type="hidden" name="booking_id" value="<?= $row['id'] ?>" /><input type="text" name="admin_notes" class="form-control form-control-sm" placeholder="Admin note" style="max-width: 140px;" /><select name="decision" class="form-select form-select-sm" style="max-width: 130px;"><option value="approved">Approve</option><option value="confirmed">Confirm</option><option value="completed">Complete</option><option value="denied">Deny</option><option value="cancelled">Cancel</option></select><button class="btn btn-sm btn-success rounded-pill" type="submit">Update</button></form></section></article></section></section><?php endforeach; ?></section></section></main>
